...
describe("product portal-config productId schema", () => {
  it("accepts a UUID string", async () => {
    ...
    try {
      ...
      // sk_test_abc isn't a valid key, but we only care that the zod schema
      // accepts the input and reaches the tool body.
  ...
  it.each([
    ...
    "rejects invalid productId at the schema boundary: %s (%j)",
    async (_label, value) => {
      const { server, client, clientTransport, serverTransport } =
        setupServer();
      await Promise.all([
